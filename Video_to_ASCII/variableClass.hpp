...
public:
	...
	void inreaseFPS() {
		auto iter = find(
			FPS_Arr.begin(),
			FPS_Arr.end(),
			static_cast<int>(this->USER_DEFINED_FPS)
		);
		int index = static_cast<int>(std::distance(FPS_Arr.begin(), iter));
	...
	void increaseResolution() {
		auto iter = find(
			...
			resolution_Arr.end(),
			static_cast<int>(this->VIEWPORT_HEIGHT)
		);

		int index = static_cast<int>(std::distance(resolution_Arr.begin(), iter));
